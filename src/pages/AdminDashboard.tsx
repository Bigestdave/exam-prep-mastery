...
export default function AdminDashboard() {
  ...
  const fetchCourses = async () => {
    ...
    if (!coursesError && coursesData) {
      const withCounts = await Promise.all(coursesData.map(async (course) => {
  ...
  const fetchCourseQuestions = async (courseId: string) => {
    const { data } = await supabase.from('course_questions').select('question_text, answer_text').eq('course_id', courseId).order('question_index', { ascending: true });
    return data ? data.map(q => ({ q: q.question_text, a: q.answer_text })) : [];
  };
  ...
  const openEditDialog = async (course: Course) => {
    ...
    const existingQs = await fetchCourseQuestions(course.id);
    setQuestions(existingQs.length > 0 ? existingQs : [{ q: "", a: "" }]);
  ...
  const handleSubmit = async () => {
    ...
    if (editingCourse) {
      ...
      const toInsert = questions.filter(q => q.q.trim() && q.a.trim()).map((q, i) => ({
        course_id: editingCourse.id, question_index: i + 1, question_text: q.q, answer_text: q.a,
      }));
      ...
      if (newCourse) {
        const toInsert = questions.filter(q => q.q.trim() && q.a.trim()).map((q, i) => ({
          course_id: newCourse.id, question_index: i + 1, question_text: q.q, answer_text: q.a,
        }));
        await supabase.from('course_questions').insert(toInsert);
      }
      toast({ title: "Created" });
  ...
  if (authLoading || adminLoading) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin" /></div>;
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="space-y-4">
              <Input value={code} onChange={e => setCode(e.target.value)} placeholder="Code (e.g. IRM 102)" />
              ...
              <Button onClick={handleSubmit} className="w-full" disabled={isSaving}>{isSaving ? "Saving..." : "Save Course"}</Button>
            </div>
          </DialogContent>
        </Dialog>
      </header>
      <main className="container py-8">
        <Table>
          <TableHeader><TableRow><TableHead>Code</TableHead><TableHead>Dept</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
          <TableBody>
            {courses.map(c => (
              <TableRow key={c.id}>
                ...
                <TableCell className="flex gap-2">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => { if(confirm("Delete?")) supabase.from('courses').delete().eq('id', c.id).then(() => fetchCourses()) }}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
